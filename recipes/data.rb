...
node[:wof_pip][:data][:metafiles].each do |f|
  ...
  execute "pull wof data for #{f}" do
    ...
    command <<-EOF
      ./bin/wof-clone-metafiles -strict -loglevel #{node[:wof_pip][:clone][:loglevel]} -dest #{node[:wof_pip][:data][:directory]} #{node[:wof_pip][:data][:dir]}/#{f} >#{node[:wof_pip][:log[:dir]}/logs/pull_wof_data.log 2>&1
    EOF
    environment(
      'HOME'    => node[:wof_pip][:user][:home],
      'PATH'    => '/bin:/usr/bin:/sbin:/usr/bin:/usr/local/go/bin:/opt/go/bin',
